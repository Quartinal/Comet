...
import * as wispFast from "wisp-server-fast";
...
/**
 * determines whether to route wisp (fast)
 * @param req the request to check on
 * @param endingUrl the wisp url (optional)
 */
function shouldRouteWispFast(req: Request, endingUrl?: string) {
  return req.url?.endsWith(endingUrl || "/wisp-fast/");
}
...
app.use((req, res, next) => {
  ...
  } else if (shouldRouteWispFast(req)) {
    wispFast.handleWebSocketConnection(req);

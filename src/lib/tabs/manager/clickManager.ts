...
export function clickHandler(event: MouseEvent) {
    ...
    if (element && element.href) {
        if (event.ctrlKey) open(event, element.href, false, false);
        else if (event.shiftKey) open(event, element.href, true, false);
        else if (element.target === "_blank") open(event, element.href, false, true);
        else if (element.target === "_parent") navigate(event, element.href);
        else if (element.target === "_top") navigate(event, element.href);
    }
}
...
export function open(
    ...
    if (isWindow && !preferences["general.tabs.openWindowLinksInTab"]) 
        window.open(`${origin}?url=${encodeURIComponent(url)}`);
    else new Tab(url, preferences["general.tabs.switchToMedia"] || isBlank);
}
...
function getAnchor(element: EventTarget | null): HTMLAnchorElement | undefined {
    ...
    else if (target.parentElement) return getAnchor(target.parentElement);
    else return;
}
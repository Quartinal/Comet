import { idleState, setIdleState } from "_tabs/data/state";
import { createBindUtil } from "_tabs/util/bindingUtil";

const bindingUtils = new Set<ReturnType<typeof createBindUtil>>();
const eventListeners = new Map<string, ((...args: any[]) => void)[]>();

export function register(worker: Worker) {
    const bindingUtil = createBindUtil(worker);
    ...
    addEventListener("visibilitychange", () => {
        if (document.visibilityState === "visible") setIdleState("active");
        else setIdleState("idle");
    });
...
function emit(event: string, ...args: any[]) {
    ...
    bindingUtils.forEach(bindingUtil => bindingUtil.emit(event, ...args));
}
...
function on(event: string, callback: (...args: any[]) => void) {
    ...
    bindingUtils.forEach(bindingUtil => bindingUtil.on(event, callback));
}

export const globalBindUtil = { emit, on };
import { keybinds, setKeybinds } from "../data/state";
...
export class Keybind {
            ...
            setKeybinds(
                keybinds.map((keybind: Keybind) => {
                    if (keybind.id === options.id) {
                        ...
                        keybind.description = 
                            options.description ?? keybind.description;
    ...
    async #init(keybind: Keybind) {
        ...
        return await store.put(
            Object.assign({}, keybind, { callback: undefined })
        );
    }
    ...
    toString() {
        ...
        str += this.key.charAt(0).toLowerCase() + this.key.slice(1);
        return str;
    }
}